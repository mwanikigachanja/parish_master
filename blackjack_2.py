...
class BlackJack:
    def __init__(self):
        ...
        self.double_down_button = tk.Button(self.root, text="Double Down", command=self.double_down, font=("Arial", 16), bg="#007bff", fg="white", relief="flat", state=tk.DISABLED)
        self.double_down_button.pack()

        self.split_button = tk.Button(self.root, text="Split", command=self.split_cards, font=("Arial", 16), bg="#007bff", fg="white", relief="flat", state=tk.DISABLED)
        self.split_button.pack()

        self.insurance_button = tk.Button(self.root, text="Insurance", command=self.take_insurance, font=("Arial", 16), bg="#007bff", fg="white", relief="flat", state=tk.DISABLED)
    ...
    def deal(self):
        ...
        self.double_down_button['state'] = tk.NORMAL
        self.split_button['state'] = tk.NORMAL
        self.insurance_button['state'] = tk.NORMAL
        ...
        if self.dealer_hand.cards[0].value == "A":
            self.result_label['text'] = "Dealer's up card is an Ace. Insurance available."
    ...
    def hit(self):
        ...
        if self.player_hand.get_value() > 21:
            ...
            self.double_down_button['state'] = tk.DISABLED
            self.split_button['state'] = tk.DISABLED
            self.insurance_button['state'] = tk.DISABLED
    ...
    def split_cards(self):
        self.split = True
        self.player_hand1 = Hand()
        self.player_hand2 = Hand()

        self.player_hand1.add_card(self.player_hand.cards[0])
        self.player_hand2.add_card(self.player_hand.cards[1])

        self.player_hand1.add_card(self.deck.deal())
        self.player_hand2.add_card(self.deck.deal())

        self.player_hand_label['text'] = f"Player Hand 1: {self.player_hand1}\nPlayer Hand 2: {self.player_hand2}"

        for widget in self.card_frame.winfo_children():
            widget.destroy()

        for card in self.player_hand1.cards:
            card_label = tk.Label(self.card_frame, text=f"{card.value} of {card.suit}", font=("Arial", 16), bg="#ffffff", fg="black", relief="raised", borderwidth=1, width=15)
            card_label.pack(side=tk.LEFT, padx=5)

        for card in self.player_hand2.cards:
            card_label = tk.Label(self.card_frame, text=f"{card.value} of {card.suit}", font=("Arial", 16), bg="#ffffff", fg="black", relief="raised", borderwidth=1, width=15)
            card_label.pack(side=tk.LEFT, padx=5)
        ...
        self.play_hand1_button = tk.Button(self.root, text="Play Hand 1", command=self.play_hand1, font=("Arial", 16), bg="#007bff", fg="white", relief="flat")
        self.play_hand1_button.pack()

        self.play_hand2_button = tk.Button(self.root, text="Play Hand 2", command=self.play_hand2, font=("Arial", 16), bg="#007bff", fg="white", relief="flat")
        self.play_hand2_button.pack()

    def play_hand1(self):
        while self.player_hand1.get_value() < 17:
            self.player_hand1.add_card(self.deck.deal())
        ...
        if self.player_hand1.get_value() > 21:
            self.result_label['text'] = "Player Hand 1 busts!"
        elif self.player_hand1.get_value() == 21:
            self.result_label['text'] = "Player Hand 1 BlackJack!"
        else:
            self.result_label['text'] = "Player Hand 1 stands."

        self.play_hand1_button['state'] = tk.DISABLED

    def play_hand2(self):
        while self.player_hand2.get_value() < 17:
            self.player_hand2.add_card(self.deck.deal())
        ...
        if self.player_hand2.get_value() > 21:
            self.result_label['text'] = "Player Hand 2 busts!"
        elif self.player_hand2.get_value() == 21:
            self.result_label['text'] = "Player Hand 2 BlackJack!"
        else:
            self.result_label['text'] = "Player Hand 2 stands."

        self.play_hand2_button['state'] = tk.DISABLED
    ...
    def take_insurance(self):
        ...
        self.player_balance -= self.bet / 2
        self.balance_label['text'] = f"Balance: ${self.player_balance}"
        self.result_label['text'] = "Insurance taken."

        if self.dealer_hand.cards[0].value == "A" and self.dealer_hand.cards[1].value in ["10", "J", "Q", "K"]:
            self.result_label['text'] = "Dealer has BlackJack! Insurance pays out."
            self.player_balance += self.bet
            self.balance_label['text'] = f"Balance: ${self.player_balance}"
        else:
            self.result_label['text'] = "Dealer does not have BlackJack. Insurance loses."
